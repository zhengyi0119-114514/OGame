...
if(NOT DEFINED __ADD_TARGET_TAG)
    ...
    function(add_target __target)
        set(__OPTIONS "WIN32;MACOSX_BUNDLE;EXCLUDE_FROM_ALL;LLVM")
    ...
    function(target_test_and_add_compile_options __target __lang __visibility)
        set(__flag_list)
        set(__cmake_required_quiet_backup "${CMAKE_REQUIRED_QUIET}")
        set(CMAKE_REQUIRED_QUIET TRUE)
        foreach(__flag IN ITEMS ${ARGN})
            ...
            string(REPLACE "-" "_" __flag_variable_name "${__flag_variable_name}")
            if("${__lang}" MATCHES [=[^C$]=])
                check_c_compiler_flag("${__flag}" "${__flag_variable_name}")
            elseif("${__lang}" MATCHES [=[^C(XX|PP|\+\+)$]=])
                check_cxx_compiler_flag("${__flag}" "${__flag_variable_name}")
            elseif("${__lang}" MATCHES [=[^(Ff)ortran$]=])
                check_fortran_compiler_flag("${__flag}" "${__flag_variable_name}")
            else()
                check_compiler_flag("${__lang}" "${__flag}" "${__flag_variable_name}")
            endif()
            if("${${__flag_variable_name}}")
                list(APPEND __flag_list "${__flag}")
            endif()
        endforeach()
        if(NOT ("${__flag_list}" STREQUAL ""))
            set(__output "${__flag_list}")
            string(REPLACE ";" " " __output "${__output}")
            message(STATUS "Target:${__target} add compile options:${__output}")
            target_compile_options("${__target}" "${__visibility}" ${__flag_list})
        endif()
        set(CMAKE_REQUIRED_QUIET "${__cmake_required_quiet_backup}")
    endfunction()
    function(target_test_and_add_link_options __target __lang __visibility)
        set(__flag_list)
        set(__cmake_required_quiet_backup "${CMAKE_REQUIRED_QUIET}")
        set(CMAKE_REQUIRED_QUIET TRUE)
        foreach(__flag IN ITEMS ${ARGN})
            check_linker_flag("${__lang}" "${__flag}" __flag_usable)
            if(${__flag_usable})
                list(APPEND __flag_list "${__flag}")
            endif()
        endforeach()
        if(NOT ("${__flag_list}" STREQUAL ""))
            set(__output "${__flag_list}")
            string(REPLACE ";" " " __output "${__output}")
            message(STATUS "Target:${__target} add link options:${__output}")
            target_link_options("${__target}" "${__visibility}" ${__flag_list})
        endif()
        set(CMAKE_REQUIRED_QUIET "${__cmake_required_quiet_backup}")
    endfunction()
endif()
