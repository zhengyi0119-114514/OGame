#include "ElectromagneticDotTimer.h"
...
} OG_CR_PV_ELECTROMAGNETIC_DOT_TIMER;

OG_CR_PTR_ELECTROMAGNETIC_DOT_TIMER *OgElectromagneticDotTimerCreate(
    int8_t iDotFrequency)
{
    if (iDotFrequency < 1)
        return NULL;
    OG_CR_PV_ELECTROMAGNETIC_DOT_TIMER *pedt = NULL;
    LARGE_INTEGER *rgliDotTable = NULL, liProcessorFrequency, liStart;
    // 分配并初始化内存
    pedt = (OG_CR_PV_ELECTROMAGNETIC_DOT_TIMER *)malloc(
        sizeof(OG_CR_ELECTROMAGNETIC_DOT_TIMER));
    rgliDotTable = (LARGE_INTEGER *)calloc(
        iDotFrequency + 1, sizeof(LARGE_INTEGER)); // NOTE: 这里多分配一个对象
    ...
    ZeroMemory((void *)pedt, sizeof(OG_CR_PV_ELECTROMAGNETIC_DOT_TIMER));
    // 取值
    ...
    {
        rgliDotTable[uIndex].QuadPart =
            liProcessorFrequency.QuadPart * uIndex / iDotFrequency;
    }
    // 赋值
    ...
    return (OG_CR_PTR_ELECTROMAGNETIC_DOT_TIMER)(pedt);
}
void OgElectromagneticDotTimerDestory(
    OG_CR_PTR_ELECTROMAGNETIC_DOT_TIMER *pPbEdt)
{
    OG_CR_PV_ELECTROMAGNETIC_DOT_TIMER *pEdt =
        (OG_CR_PV_ELECTROMAGNETIC_DOT_TIMER *)pPbEdt;
    if (pEdt)
    {
...
int64_t OgElectromagneticDotTimerPullTask(
    OG_CR_PTR_ELECTROMAGNETIC_DOT_TIMER *pPbEdt)
{
    OG_CR_PV_ELECTROMAGNETIC_DOT_TIMER *pEdt =
        (OG_CR_PV_ELECTROMAGNETIC_DOT_TIMER *)pPbEdt;
    ...
    liRemainingPart.QuadPart =
        liTimeDifference.QuadPart % pEdt->liProcessorFrequency.QuadPart;
    iResult += liTimeDifference.QuadPart / pEdt->liProcessorFrequency.QuadPart *
               pEdt->iDotFrequency;
    iResult += liRemainingPart.QuadPart * pEdt->iDotFrequency /
               pEdt->liProcessorFrequency.QuadPart;
    return iResult;
}
void OgRefreshElectromagneticDotTimer(
    OG_CR_PTR_ELECTROMAGNETIC_DOT_TIMER *pPbEdt)
{
    OG_CR_PV_ELECTROMAGNETIC_DOT_TIMER *pEdt =
        (OG_CR_PV_ELECTROMAGNETIC_DOT_TIMER *)pPbEdt;
...
void OgElectromagneticDotTimerSkipATimeStamp(
    OG_CR_PTR_ELECTROMAGNETIC_DOT_TIMER *pPbEdt)
{
    OG_CR_PV_ELECTROMAGNETIC_DOT_TIMER *pEdt =
        (OG_CR_PV_ELECTROMAGNETIC_DOT_TIMER *)pPbEdt;
    ...
    liTimeDifference.QuadPart %= pEdt->liProcessorFrequency.QuadPart;
    for (uint16_t uIndex = 0, uIndexMax = pEdt->iDotFrequency + 1;
        uIndex < uIndexMax; uIndex++)
    {
        if ((pEdt->rgliDotTable[uIndex].QuadPart) < liTimeDifference.QuadPart &&
            (liTimeDifference.QuadPart <=
                (liCache.QuadPart = pEdt->rgliDotTable[uIndex + 1].QuadPart)))
        {
            liTimeSleepTarget.QuadPart =
                liCache.QuadPart - liTimeDifference.QuadPart;
        }
    }
    liCache.QuadPart = liCurrentTime.QuadPart + liTimeSleepTarget.QuadPart;
    dwMillisecondsToSleep =
        liTimeSleepTarget.QuadPart * 1000 / pEdt->liProcessorFrequency.QuadPart;
    Sleep(dwMillisecondsToSleep);
    while (TRUE)
    {
        QueryPerformanceCounter(&liCache);
        if (liCache.QuadPart - liCurrentTime.QuadPart >=
            liTimeSleepTarget.QuadPart)
        {
            break;
        }
...
OG_MACRO_PRIVATE inline void TimespecDifference(struct timespec *ptsSubtrahend,
    struct timespec *ptsReduction,
    struct timespec *ptsOutput);
OG_MACRO_PRIVATE inline int64_t TimespecToInt64(struct timespec *pts);
OG_MACRO_PRIVATE inline struct timespec Int64ToTimespec(int64_t i);
...
void TimespecDifference(struct timespec *ptsSubtrahend,
    struct timespec *ptsReduction,
    struct timespec *ptsOutput)
{
...
OgElectromagneticDotTimer *OgElectromagneticDotTimerCreate(
    int8_t iDotFrequency)
{
    size_t uSizeOfEdtStruct = sizeof(OG_CR_PV_ELECTROMAGNETIC_DOT_TIMER);
    size_t uSizeOfDotTable =
        sizeof(clock_t) * (iDotFrequency + 1); /*<<这里故意多添加了一个元素 */
    OG_CR_PV_ELECTROMAGNETIC_DOT_TIMER *pEdt;
    // 检pane spl查参数是否有效
    if (iDotFrequency < 1)
        return NULL;
    ...
    // 分配并初始化内存
    pEdt = (OG_CR_PV_ELECTROMAGNETIC_DOT_TIMER *)malloc(uSizeOfEdtStruct);
    if (!pEdt)
        return NULL;
    memset((void *)pEdt, 0, uSizeOfEdtStruct);
    pEdt->piDotTable = (clock_t *)malloc(uSizeOfDotTable);
    if (!pEdt->piDotTable)
    {
        free(pEdt);
        return NULL;
    }
    memset((void *)pEdt->piDotTable, 0, uSizeOfDotTable);
    pEdt->iDotFrequency = iDotFrequency;
    // 填充时间表
    for (uint32_t uIndex = 1; uIndex < iDotFrequency + 1; uIndex++)
    {
        pEdt->piDotTable[uIndex] =
            NUMBER_OF_NS_IN_PRE_SECOND * uIndex / iDotFrequency;
    }
    return (OgElectromagneticDotTimer *)pEdt;
}
void OgElectromagneticDotTimerDestory(
    OgElectromagneticDotTimer *pPbEdt)
{
    OG_CR_PV_ELECTROMAGNETIC_DOT_TIMER *pEdt =
        (OG_CR_PV_ELECTROMAGNETIC_DOT_TIMER *)pPbEdt;
    if (!pEdt)
...
OgUnsignedInteger64 OgElectromagneticDotTimerPullTask(
    OgElectromagneticDotTimer *pPbEdt)
{
    OG_CR_PV_ELECTROMAGNETIC_DOT_TIMER *pEdt =
        (OG_CR_PV_ELECTROMAGNETIC_DOT_TIMER *)pPbEdt;
    ...
    {
        if (iNsDifference >= pEdt->piDotTable[sIndex] &&
            iNsDifference < pEdt->piDotTable[sIndex + 1])
        {
            return sIndex;
        }
    }
    ...
    OgElectromagneticDotTimer *pPbEdt)
{
    ...
    int64_t iTimeDifferenceNanoSecond = 0, iTimeToSleep = 0;
    clock_gettime(CLOCK_MONOTONIC, &tsCurrentTime);
    // 计算应等待的时间
    ...
    for (size_t sIndex = 0; sIndex < pEdt->iDotFrequency + 1; sIndex++)
    {
        ...
        {
            iTimeToSleep =
                pEdt->piDotTable[sIndex + 1] - iTimeDifferenceNanoSecond;
            break;
        }
    }
