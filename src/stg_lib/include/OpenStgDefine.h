...
#define OPEN_GAME_STRUCT_DEFINE_H 1
    ...
    #define OG_ALWAYS_INLINE __attribute__((always_inline))
#else
...
#define OPEN_STG_ERROR_MESSAGE_MODULE_NOT_EXIST                 ((OG_ERROR_CODE_T)OG_ERROR_CODE_C(0x00000006))
#define OPEN_STG_ERROR_MESSAGE_NOT_INITIALIZED                  ((OG_ERROR_CODE_T)OG_ERROR_CODE_C(0x00000007))
...
#define OPEN_STG_REGISTRAR_NAME_CORE                                    "ice_thorn.core.cirno"
#define OPEN_STG_REGISTRAR_NAME_GRAPHICS                                "ice_thorn.graphics.baka"
/**
 * @brief 注册模块时的参数
 *
 * //NOTE: 改动时手动更改OgCrRegisterProgramModule()函数
 *
 * @note pfFreeFunc先于pfDestoryProgramModule调用
 */
typedef struct TagOgCrPROGRAM_MODULE
{
    ...
    OG_CR_FORMAT_ERROR_MESSAGE_FUNCTION_T pfFormatErrorMessage; ///< (REQUIRED)输入，获取错误码对应的字符串描述函数
    OG_CR_DESTORY_PROGRAM_MODULE_T pfDestoryProgramModule;      ///< 输入，模块的销毁函数
    void *pvAdditionalData;                                     ///< 输入，模块额外信息
    OG_CR_MODULE_ADDITIONAL_RELEASE_FUNCTION_T pfFreeFunc;      ///< (REQUIRED)输入，模块额外信息释放函数
...
typedef struct TagOgCrUNIVERSAL_REGISTRAR_INFORMATION
{
    ...
    uint32_t uItemStructureSize;                    ///< 输入，单位结构大小
    uint32_t uPreAllocatedCount;                    ///< 输入，预分配结构大小
 ...
 * @brief 程序模块指针
 */
typedef void* PTR_PROGRAM_MODULE;

typedef void (OG_API* OG_CR_EXCEPTION_HANDLER_FUNCTION_T)(OG_ERROR_T e);
#if defined __cplusplus
}
#endif
#endif
