...
OG_CR_THREAD_LOCAL_STORAGE_STRUCT *OgCrGetTLSStruct(void)
{
    ...
    if (ptlss == NULL)
    {
        size_t uSizeOfTLSSturct = sizeof(OG_CR_THREAD_LOCAL_STORAGE_STRUCT);
        ptlss = (OG_CR_THREAD_LOCAL_STORAGE_STRUCT *)malloc(uSizeOfTLSSturct);
        memset((void *)ptlss, 0, uSizeOfTLSSturct);
