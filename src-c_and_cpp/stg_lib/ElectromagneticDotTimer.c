...
{
    size_t uSizeOfEdtStruct = sizeof(OG_CR_ELECTROMAGNETIC_DOT_TIMER);
    size_t uSizeOfDotTable = sizeof(clock_t) * (iDotFrequency + 1); /*<<这里故意多添加了一个元素 */
    ...
    // 分配并初始化内存
    pEdt = (OG_CR_ELECTROMAGNETIC_DOT_TIMER *)malloc(uSizeOfEdtStruct);
    if (!pEdt)
        return NULL;
